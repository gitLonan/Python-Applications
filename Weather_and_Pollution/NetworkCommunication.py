...
class NetworkCommunication:
    ...
    def get_latitude_longitude(user)-> tuple[float, float]:
        ...
        try:
            response = NetworkCommunication.url_access(url)
    ...
    def get_current_weather_data(latitude: float, longitude: float, user):
        ...
        if response:
            result = json.loads(response)
            #print(json.dumps(result, indent=2))                    
            with open(r"all the json files/Current Weather.json", "w") as f:
    ...
    def get_forecast_weatherFor_5days_data(latitude: float, longitude: float, user):
        ...
        if response:
            result = json.loads(response)
            #print(json.dumps(result, indent=2))                    
            with open(r"all the json files/Forecast_weather_3h_step_5days.json", "w") as f:
                json.dump(result, f, indent=2)
        return
    ...
    def get_current_pollution(latitude: float, longitude: float, user):
        ...
        if response:
            result = json.loads(response)
            #print(json.dumps(result, indent=2))                    
            with open(r"all the json files/Current Pollution.json", "w") as f:
                json.dump(result, f, indent=2)
        return
    ...
    def get_forcast_air_polution(latitude: float, longitude: float, user):
        ...
        if response:
            result = json.loads(response)
            #print(json.dumps(result, indent=2))                    
            with open(r"all the json files/Forecast Pollution.json", "w") as f:
                json.dump(result, f, indent=2)
