...
class Parsed:
    ...
    def get_days_for_forecast(ID):
        ...
        with open(f"all the json files/{folderFrom_which_to_scrape}.json", "r") as file:
            data = json.load(file)
    ...
    def loading_current_weather(user):
        ...
        with open(r"all the json files/Current Weather.json", "r") as f:
            data = json.load(f)
            for key, value in data.items():
    ...
    def loading_pollution():
        with open(r"all the json files/Current Pollution.json", "r") as f:
            data = json.load(f)
    ...
    def loading_forecast_pollution(days_pollution): 
        ...
        with open(r"all the json files/Forecast Pollution.json", "r") as file:
            data = json.load(file)
    ...
    def loading_forecast_weather(days_weather,user):
        ...
        day_index = 0
        checking_for_next_day = 0
        with open(r"all the json files/Forecast_weather_3h_step_5days.json", "r") as file:
            data = json.load(file)
            #get to the list that is bascially the whole file
            for key, value in data.items():
                if type(value) == list:
